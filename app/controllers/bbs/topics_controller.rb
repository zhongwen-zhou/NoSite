# coding: utf-8
class Bbs::TopicsController < Bbs::BaseController

  # load_and_authorize_resource :only => [:new, :edit, :create, :update, :destroy,:favorite, :follow, :unfollow, :suggest, :unsuggest]
  load_and_authorize_resource :only => [:new, :edit, :update, :destroy,:favorite, :follow, :unfollow, :suggest, :unsuggest]
  ...
  def index
    @topics = ::Bbs::Topic.last_actived.without_hide_nodes.fields_for_list.includes(:user).paginate(:page => params[:page], :per_page => 15, :total_entries => 1500)
    set_seo_meta("","#{Setting.app_name}#{t("menu.topics")}")
    drop_breadcrumb(t("topics.topic_list.hot_topic"))
  end

  def feed
    @topics = ::Bbs::Topic.recent.without_body.limit(20).includes(:node,:user, :last_reply_user)
    render :layout => false
  end
  ...
  %w(no_reply popular).each do |name|
    define_method(name) do
      @topics = ::Bbs::Topic.send(name.to_sym).last_actived.fields_for_list.includes(:user).paginate(:page => params[:page], :per_page => 15, :total_entries => 1500)
  ...
  def recent
    @topics = ::Bbs::Topic.recent.fields_for_list.includes(:user).paginate(:page => params[:page], :per_page => 15, :total_entries => 1500)
  ...
  def excellent
    @topics = ::Bbs::Topic.excellent.recent.fields_for_list.includes(:user).paginate(page: params[:page], per_page: 15, total_entries: 500)
  ...
  def show
    ...
    drop_breadcrumb("#{@node.try(:name)}", bbs_node_topics_path(@node.try(:id)))
  ...
  def new
    ...
    if !params[:node].blank?
      ...
      @node = ::Bbs::Node.find_by_id(params[:node])
      if @node.blank?
        render_404
      end
      drop_breadcrumb("#{@node.name}", bbs_node_topics_path(@node.id))
    end
  ...
  def create
    @topic = ::Bbs::Topic.new(topic_params)
  ...
  def init_base_breadcrumb
    drop_breadcrumb(t("menu.topics"), bbs_topics_path)
  end
  ...
  def init_list_sidebar
    if !fragment_exist? "topic/init_list_sidebar/hot_nodes"
      @hot_nodes = ::Bbs::Node.hots.limit(10)
    end
    set_seo_meta(t("menu.topics"))
  end

  def topic_params
    params.require(:bbs_topic).permit(:title, :body, :node_id)
  end
end

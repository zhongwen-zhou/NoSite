...
class Bbs::TopicsCell < BaseCell
  ...
  def index_sections
    @sections = ::Bbs::Section.all #.includes(:nodes)
    render
  end
  ...
  def sidebar_statistics
    ...
    @topics_count = ::Bbs::Topic.unscoped.count
    @replies_count = ::Bbs::Reply.unscoped.count
    render
  end
  ...
  def sidebar_suggest_topics
    @suggest_topics = ::Bbs::Topic.suggest.limit(5)
    render
  end
